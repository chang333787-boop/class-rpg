...
> **바뀜(DECO-KINDS-1 · 2026-09-26 사용자 "항목별로 구분하면 더 눈에 잘 보이지 않을까 · 찾기 칸이 긴데 그 공간 좀 쓰지")** — 분류 칩은 이제 **두 탭이 같이 쓰는 머리줄 안 한 자리**다.
> - 넓은 화면(901px~): 찾기 칸을 210px 로 줄이고(1366 에서 964px 이던 칸) 그 옆 남는 폭 전부가 칩. 머리줄은 한 줄 그대로 → 상점 탭 서랍이 한 줄(50px) 낮아졌다(1366×610: 서랍 209 → 159px).
>   901~1279 에서 상점 카드를 고르면 사기 막대만 머리 아래 한 줄로 내려간다(그때 서랍 키 = 옛 상점 그대로).
> - 좁은 화면(~900px): 머리 아래 한 줄(옛 상점 칩 줄 자리). '내 것'은 **⌃ 로 펼쳤을 때만** — 접힌 서랍에 한 줄 더하면 폰 마당이 화면 절반 밑(45%)으로 떨어진다. 키 600px 밑(756×469 등)은 펼쳐도 안 보인다(펼친 서랍이 위 단추를 덮었다). 이때도 묶음 이름표는 있다.
> - 고른 종류는 두 탭이 같이 쓴다(🌳 나무를 보다가 🛒 로 가면 나무 상점). 그 장소에 없는 종류면 전체로. 칩이 넘치면 오른쪽 끝이 흐려진다.
> - **'전체'로 볼 때는 종류끼리 모아 묶음 앞에 작은 이름표**(세로 글 `🌳 나무`)를 붙인다 — 내 것·상점 둘 다. 이름표는 누르는 것이 아니다(거르기는 칩).
> - 내 것 칩은 '이 장소만'까지 거른 가방에 있는 종류만. '전체' + 한 종류뿐이면 칩을 감춘다.
...
|---|---|---|---|---|
| 1366×610 (크롬북) | **1줄**(나머지는 서랍 안에서 넘김) | 머리 오른쪽 | ~~찾기 줄 아래 한 줄~~ → 머리줄 안 찾기 칸 옆(DECO-KINDS-1) | 상점 탭 ~~55%~~ → 내 것 탭과 같음 · 내 것 탭 65% |
