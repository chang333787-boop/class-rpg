...
## 섬 아래 몸 다시 그림 (2026-09-24 · 프로그램 담당: '가로 줄무늬처럼 보인다')
![위 옛것(판 폭 층 선 셋) · 가운데 새것 · 아래 새것 칸 27px](img/deco_star_yard_20260924/island_under_v2.png)

- 옛 그림은 층 선 셋이 판 폭(80칸)을 **한 줄로 가로질러** 줄무늬가 되었다.
- 새 그림에서는 판 폭을 가로지르는 선을 없앴다(0개):
  - 엇갈린 짧은 바위 덩이
  - 세로 금
  - 아래 끝을 따라가는 그늘
- 크기(1600×120) · 자리 · 이름은 그대로이고, 무게는 4.5KB 다.

## 실화면 점검 뒤 테 · 땅 다시 (2026-09-24 · 프로그램 P5 #1094 뒤 `?look=star`)
![전후 — 칸 27 · 54](img/deco_star_yard_20260924/rim_ground_v2.jpg)

- **섬 아래 몸**: #1086 새 그림이 쓰이고 있다. 판 폭 가로선은 없다.
- **테 `island_rim`**: 칸마다 같은 자리의 수정 점, 가운데 물결선, 아래 짙은 선이 판 폭으로 이어져 **점선 · 가로줄**로 보였다(#1094 사진의 줄).
  - 고침: 밝은 윗입술 하나에, 아래로 어두워지는 몸을 두어 아래 몸으로 이어지게 했다.
- **땅 `star_ground`**: 빈 곳에서 9칸마다 **같은 별 무리 · 짙은 결**이 되풀이되어 보였다.
  - 고침: 결을 크고 옅게(명암 차 ↓) 하고 별 무리를 뺐다. 별은 은하수 띠가 맡는다.
- 이름 · 크기 · 주기는 그대로다. 코드 변경은 0 이다.
