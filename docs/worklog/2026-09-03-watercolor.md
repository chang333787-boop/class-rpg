...
## 대비 자료 — 차시 번호 변경(17→16, #154) 기록 마이그레이션 매핑표 (보스 지시, 코드는 아직 안 씀)
#145(17차시) 배포 후 #154 머지 전에 학생이 남긴 기록이 있을 때만 별도 PR로 마이그레이션. 사용자가 "오늘 실사용 안 했다"면 불필요.

| 옛 id | 옛 제목 | 새 id | 새 제목 | 규칙 |
|---|---|---|---|---|
| 0 | 준비하기 | 0 | 준비하고 붓과 놀기 | 그대로(단계 0~4 동일) |
| 1 | 붓과 놀기 | 0 | (합쳐짐) | 새 0의 단계 5~6에 해당 → progress = max(p0, p1 ? p1+5 : 0). 성찰은 옛 1 우선 |
| 2 | 우연 효과 놀이 | 8 | 질감과 우연 효과 | 새 8의 단계 3~5에 해당 → progress = max(p10, p2 ? min(p2,2)+3 : 0). 성찰은 옛 10 우선 |
| 3 | 물의 양과 진하기 | 1 | 〃 | 이름만 변경 |
| 4 | 색 섞기 | 2 | 〃 | 〃 |
| 5 | 평칠과 그러데이션 | 3 | 〃 | 〃 |
| 6 | 번지기 | 4 | 〃 | 〃 |
| 7 | 겹쳐 칠하기 | 5 | 〃 | 〃 |
| 8 | 크레파스 배수 | 6 | 〃 | 〃 |
| 9 | 작품: 저녁 하늘과 언덕 | 7 | 〃 | 〃 |
| 10 | 질감 효과 | 8 | 질감과 우연 효과 | 단계 0~2 동일(옛 2와 합침, 위 규칙) |
| 11 | 색상환과 보색 | 9 | 〃 | 이름만 변경 |
| 12 | 명암과 입체(공) | 10 | 명암과 입체: 띠로 칠하기 | **내용 재작성** — 완료 여부(step ≥ 4)만 옮기고 중간 진행은 0으로 |
| — | — | 11 | 나무 그리기 | 신설, 옛 기록 없음 |
| 13 | 멀리와 가까이 | 12 | 〃 | 이름만 변경 |
| 14 | 관찰해서 그리기 | 13 | 〃 | 〃 |
| 15 | 마음을 색으로 | 14 | 〃 | 〃 |
| 16 | 나만의 작품 | 15 | 〃 | 〃 |

대상 저장소 4곳: ① localStorage `wc.progress.<sid>`·`wc.reflect.<sid>`(키 = 옛 id) ② IndexedDB `wc`/photos의 `lesson` 필드 ③ Firebase `classRPG_watercolor/<sid>`의 `progress`·`reflect` ④ 작품 pendingReward의 `artTitle`("수채화 12. 명암과 입체")은 문자열이라 손대지 않음(교사 승인 화면 표시용).
실행 방식(안): 페이지 로드 시 `localStorage['wc.schema'] !== '2'`이면 ①②를 한 번 변환하고 `wc.schema=2` 기록; RPG 로그인 후 `rpgLoad` 때 Firebase 노드에 `schema` 없으면 ③ 변환 후 `schema:2`로 set(부분 set, 자기 노드만). 영상 파일명은 이미 rename 완료라 무관.
