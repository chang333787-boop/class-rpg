...
## 손님 선물 도토리 · 깃털 다시 그림 (2026-09-25 · 선물 상자 규격)
![전 · 후 — 달걀 · 나뭇가지 · 사과 · 버찌와 나란히 18 · 32 · 64px](img/deco_guests_art_20260924/guest_gifts.jpg)

- 규격은 다른 선물과 같다:
  - 100×100 · 땅 그림자 타원 `cy84 · ry5` · 갈색 윤곽 2.5 · 몸 폭 40~52
  - 코드 `.deco-gift`(칸의 .62 · 최소 18px)와 선물 상자 칸(32px)에서 그대로 쓴다.
- `gift_acorn`:
  - 폭 36→38, 모자 44→48. 모자 비늘 결 · 꼭지 윤곽 · 몸 그늘을 넣었다.
  - 그림자를 `cy86 · ry4` 에서 `cy84 · ry5` 로 다른 선물과 맞췄다.
- `gift_feather`:
  - **땅 그림자가 없었다** → 더했다.
  - 가는 펜처럼 보여 깃 폭을 넓혔다. 끝 밝은 결 · 흰 깃대 · 깃 결 여섯을 넣어 18px 에서도 '깃털'로 읽힌다.
- 이름 · 파일 이름 · 코드는 그대로다(`LIFE_GIFT_NAME` 의 도토리 · 깃털).
