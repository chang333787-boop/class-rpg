...
| 그림 액자 `d_i4` | 벽걸이 그대로. **벽걸이 모양 다듬기는 그림 할 일**(§4) |
| 벽지·바닥 고르기 화면 | 마당 바닥 고르기 (다)와 **같은 부품**(§3) |
| 방 밖 빈 터 | **사용자 선택 대기** — (A) 어두운 도면 추천(디자인·보스 같음). 코드는 스위치 하나로 세 안을 다 받는다(§1) |
...
`INDOOR_OUTSIDE = 'blueprint' | 'oldfloor' | 'concrete'` — 사용자가 고른 값 하나로 고정(아이가 고르는 것 아님 · 저장 0).

| 값 | 생김새 | 그리는 법 |
|---|---|---|
| **`blueprint`** (A) 추천 | 짙은 남색 + 옅은 칸줄, 5칸마다 조금 진하게. 방이 등불 켠 것처럼 떠오른다 — '도면 위에 방을 짓는다' | 바탕 `#161b28` · 칸줄 1px `rgba(140,170,220,.09)` · 5칸마다 `rgba(140,170,220,.2)` |
...
| 장식 | 규칙 | 그림 |
|---|---|---|
| 그림 액자 `d_i4` · (IN-4 의 창·선반·벽등) | 저장은 **방 윗줄 그대로**, 그림은 **한 칸 위 벽 띠에** | 지금 그림은 바닥에 세운 모양(바닥 그림자) → 시안은 액자 부분(몸통 viewBox y 110~176)만 잘라 폭 `0.78C`, 벽 띠 위 `0.12C` 아래에 걸고 그림자 `2px rgba(0,0,0,.18)`. **그림 할 일: `d_i4` 벽걸이 모양**(벽 그림자 · 띠 높이에 맞춘 비율) — 생기면 자르기 대신 그 그림 |
