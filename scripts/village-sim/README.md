...
- FAIL: **오래 포갬**(같은 둘이 그려진 자리 0.6 안에 1초 넘게 · 20프레임 넘게 — 긴 프레임이 몰리면 프레임마다 벌리는 간격이 못 따라온다) · 줄 세움(가장 가까운 같은 둘이 두 표본 잇달아 정확히 4.00 — 스치는 한순간은 뺌) · 벽 통과(`--first30` 과 같은 몸 높이 부위 상자) · 튐(같은 상태로 한 프레임 2.0 넘게 · 100ms 넘는 긴 프레임 뒤는 '긴 프레임'으로 따로) · 갇힘(`__stuck` — until 로 나오는 '들름'만 · 집·일터·학교는 시각으로 나와 뺀다) · 오류(페이지 예외 + `__errors` 의 삼킨 오류·덮개).
...
- **`--input`**(09-25): 판마다 잰 뒤 **입력 단계**를 한 번 돈다(판이 바뀌므로 잰 것 뒤에). 수업 판 시작 카드는 '건너뛰기'로 닫는다.
  - 흰 공: 풍선 화면 지름 px(얼굴 2.2 · 말 표 3.6 × 한칸px ÷ 4)입니다. 얼굴이 24px 아래면 **주의**(창조자 25회 ⓑ48 제안 기준)이고, 지금 기본 줌은 17~19px 입니다.
  - 누름: 화면 안 사는 집 하나를 CDP 마우스로 누르고, `#toast`·`#why` 글이 처음 바뀐 때까지 ms 를 잽니다. 150ms 넘으면 **주의**입니다.
  - 치우기: 🧽 를 켜고 빈 집 하나를 한 번 누릅니다. 1초 안에 `#toast` 와 `#why` 가 **둘 다 다른 글**이면 **말 충돌 FAIL**입니다(ⓐ4 '집을 치웠어요' ＋ '여기엔 지울 게 없어요' · [MAC-ONEPRESS] 를 끄면 pop167·sea 에서 다시 잡힘).
  - 누르기 전에 앞의 토스트(🎯·🔓)가 사라질 때까지 기다립니다(10초까지 · 못 기다리면 '⚠앞말'). `(셈 포갬 표본)` 은 창조자 25회 표와 견주려는 셈 자리(2.0 안) 값입니다.
