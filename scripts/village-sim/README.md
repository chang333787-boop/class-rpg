...
- FAIL: **오래 포갬**(같은 둘이 그려진 자리 0.6 안에 1초 넘게) · 줄 세움(정확히 4.00) · 벽 통과(`--first30` 과 같은 몸 높이 부위 상자) · 튐(같은 상태로 한 프레임 2.0 넘게 · 100ms 넘는 긴 프레임 뒤는 '긴 프레임'으로 따로) · 갇힘(`__stuck` — until 로 나오는 '들름'만 · 집·일터·학교는 시각으로 나와 뺀다) · 오류(페이지 예외 + `__errors` 의 삼킨 오류·덮개).
