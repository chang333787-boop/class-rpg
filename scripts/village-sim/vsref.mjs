...
//   --more <판,판>(표준 판에 더함) · --soft <판이름,…>(그 판은 옛과 달라도 FAIL 이 아니라 '⚠ 바뀐 판 · 의도한 차이? — PR 에 적기') — village-check 가 PR 이 바꾼 판에 쓴다
//   옛(ref)에 판 파일이 없는 **새 판**은 옛과 견주지 않는다('새 판 — 옛에 없음' · 흔들기만)
...
const BOARDS = [...new Set(opt('boards', `기본+village/stages/boards/pop88.json,기본+village/stages/boards/pop167.json,farm+${MID},farm,city,town3,town3-origin,origin,sea,mountain,proto-flow,proto-vote`).split(',').concat(String(opt('more', '')).split(',')).map(s => s.trim()).filter(Boolean))];   // [MAC-CHECKCHANGED] --more 는 더하기
const SOFT = new Set(String(opt('soft', '')).split(',').map(s => s.trim()).filter(Boolean));
...
for (const spec of BOARDS) {
  ...
  const isNew = name !== '기본' && !fs.existsSync(path.join(tmp, 'village/stages', name + '.json'));   // 옛에 판 파일이 없다 — 견줄 것이 없는 새 판
  const a = ['--days', DAYS, '--seeds', SEEDS, ...(isNew ? [] : ['--vs', '옛: root=' + tmp]), '--json', out];
  ...
  const old = isNew ? { 값: 0, 다른값: 0, 처음: null } : cmp('옛'), look = LOOK ? cmp('흔듦') : null;
  rows.push({ 판: boardName(spec), 새판: isNew, 바뀐판: SOFT.has(boardName(spec)) || SOFT.has(name), 값: old.값, 옛다름: old.다른값, 옛처음: old.처음, 흔듦다름: look ? look.다른값 : null, 흔듦처음: look ? look.처음 : null });
...
let oldBad = 0, lookBad = 0, failed = 0; const softDiff = [];
for (const r of rows) {
  if (r.실패) { failed++; L.push(`| ${r.판} | — | 실패 |${LOOK ? ' — |' : ''} ${r.실패} |`); continue; }
  if (r.새판) { if (r.흔듦다름) lookBad++; L.push(`| ${r.판} (새 판) | — | 옛에 없음 |${LOOK ? ' ' + (r.흔듦다름 ? '**' + r.흔듦다름 + '**' : 0) + ' |' : ''} 새 판 — 옛에 판 파일이 없어 견줄 것이 없음 |`); continue; }
  if (r.옛다름 && r.바뀐판) softDiff.push(r.판 + ' ' + r.옛다름 + '값'); else if (r.옛다름) oldBad++; if (r.흔듦다름) lookBad++;
  const first = r.옛처음 ? r.옛처음 : r.흔듦처음 ? '흔들기 · ' + r.흔듦처음 + '(흔든 쪽 → 지금)' : '—';   // cmp 가 이미 '비교 쪽 → 지금' 순서로 적는다
  L.push(`| ${r.판}${r.바뀐판 ? ' (바뀐 판)' : ''} | ${r.값} | ${r.옛다름 ? '**' + r.옛다름 + '**' : 0} |${LOOK ? ' ' + (r.흔듦다름 ? '**' + r.흔듦다름 + '**' : 0) + ' |' : ''} ${first} |`);
}
const verdict = failed ? 'FAIL — 못 돈 판 ' + failed : lookBad ? 'FAIL — 그림 난수를 흔들었더니 판정 값이 움직인 판 ' + lookBad + ' (판정 코드가 Math.random 을 쓰나? #926)'
  : oldBad ? (ALLOW ? 'PASS(허용) — 옛과 다른 판 ' + oldBad + ' · PR 에 의도한 차이로 적을 것' : 'FAIL — 옛과 다른 판 ' + oldBad + ' (의도한 차이면 --allow 로 · PR 에 적기)') : 'PASS — ' + (softDiff.length ? '바뀐 판 말고는 ' : '') + '모든 판에서 옛과 같고' + (LOOK ? ', 그림 난수를 흔들어도 판정이 같다' : '');
L.push('', '판정: ' + verdict);
if (softDiff.length) L.push(`⚠ 바뀐 판이 옛과 다름 — 의도한 차이? PR 에 적기: ${softDiff.join(' · ')}`);   // [MAC-CHECKCHANGED] FAIL 로 막지 않고 눈에 띄게
console.log(L.join('\n'));
